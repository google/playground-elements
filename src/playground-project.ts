...
export class PlaygroundProject extends LitElement {
  ...
  `;
    ...
    async updateCompletions(
        ...
        if (!codeEditorChangeData.isRefinement) {
            ...
            if (completionInfo) {
                ...
                this._completionInfo = this._populateCompletionInfoWithDetailGetters(
                    completionInfo,
                    filename,
                    cursorIndex
                );
        ...
        const previousFirstCompletion = this._completions?.[0];
        ...
        // If the previous 0-index completion was the same as this time, we can
        // re-use the fetched detail from there, reducing round trips
     ...
     * function build in, so that the code editor can use it to fetch
     * the details when needed itself, instead of having to ask the project
     * layer for them.
     */
    _populateCompletionInfoWithDetailGetters(
        completionInfo: CompletionInfo,
        filename: string,
        cursorIndex: number
    ) {
        const completionInfoWithDetails =
            completionInfo as CompletionInfoWithDetails;
        const getCompletionDetailsFunction = this.getCompletionDetails.bind(this);
