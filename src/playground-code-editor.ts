...
export class PlaygroundCodeEditor extends LitElement {
  ...
  get cursorPosition(): EditorPosition {
    if (!this._editorView) return {ch: 0, line: 0};
  ...
  get tokenUnderCursor(): EditorToken {
    ...
    // Simple tokenization - find word boundaries around cursor
    ...
    return {
      ...
      string: lineText.slice(start, end),
    };
  }
  ...
  override update(changedProperties: PropertyValues) {
    ...
    // We'll collect effects that need to be dispatched
    ...
    for (const prop of changedTyped.keys()) {
      switch (prop) {
        case 'documentKey': {
          ...
          // Cache the current state before switching to a new document
          if (lastKey && this._editorView) {
            const lastState = this._editorView.state;
            this._docCache.set(lastKey, lastState);

            // Update the state's value before we switch to a new document
          ...
          if (!docState) {
            // Create a new EditorState for this document
            ...
            // If we have a cached state but its content doesn't match the current value,
            // update it with a transaction instead of creating a new state to preserve history
        ...
        case 'value':
          ...
          if (this.value !== view?.state.doc.toString()) {
            ...
            // Create a transaction that replaces the document content
            view?.dispatch({
    ...
    // Apply any effects to change the state config if we have them
    if (effects.length > 0) {
  ...
  private _currentFiletypeSupportsCompletion() {
    ...
    // You could expand this to support more languages as shown in the docs
            ...
            async (context) => {
              ...
              const isRefinement =
                ...
                (word?.text?.startsWith('.') ||
                  (wasRecent && (word?.text?.length ?? 0) > 1));

              let resolve: (
                value: EditorCompletion[] | PromiseLike<EditorCompletion[]>
              ) => void;
              const completionsPromise = new Promise<EditorCompletion[]>(
                (res) => {
                  resolve = res;
                }
              );
  ...
  private async _applyHideAndFoldRegions() {
    ...
    // Create decorations for hide/fold regions
    const decorations: Range<Decoration>[] = [];
    const text = this._editorView.state.doc.toString();
    // Define a single annotation type that will carry the fold ID to unfold
    ...
    for (const match of text.matchAll(pattern)) {
      ...
      const foldId = openerStart; // Use the start position as a unique identifier
      ...
      if (this.pragmas === 'on') {
        if (kind === 'fold') {
          // Add fold widget
          // Extract the content after the fold for display
          ...
          decorations.push(
            Decoration.replace({
              widget: widget,
              kind: 'fold',
              foldId, // Store the fold ID in the decoration
    ...
    const hideAndFoldField = StateField.define<DecorationSet>({
      ...
      update: (deco, transaction) => {
        // Check if the unfold annotation is present
        ...
        if (foldIdToRemove !== undefined) {
          ...
          // Create a new decoration set without the specific fold
          const newDecorations: Range<Decoration>[] = [];
          deco.between(0, Infinity, (from, to, value) => {
            // Only add decorations that don't match the fold ID we want to remove
            if (
              !(
  ...
  // CM6's language extensions are handled in _getLanguageExtension method
